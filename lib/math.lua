...
function math.in_circle(px, py, cx, cy, rs)
...
-- math.in_polygon(0, 0, -1, -1, 1, -1, 0, 2) -> true
-- math.in_polygon(10, 10, -1, -1, 1, -1, 0, 2) -> false
-- math.in_polygon(-1, -1, -1, -1, 1, -1, 0, 2) -> true
function math.in_polygon(px, py, ...)
...
function math.generate_curves(points, tension, continuity, bias)
    ...
    if #points / 2 >= 3 then
        local curves = {}
        table.insert(curves,
            kochanek_bartels(points[1], points[2], points[1], points[2], points[3], points[4], points[5], points[6],
                tension, continuity, bias))
        for i = 2, #points / 2 - 2 do
            local j = i * 2 - 1
            table.insert(curves,
                kochanek_bartels(points[j - 2], points[j - 1], points[j], points[j + 1], points[j + 2], points[j + 3],
                    points[j + 4], points[j + 5], tension, continuity, bias))
        end
        local n = #points
        table.insert(curves,
            kochanek_bartels(points[n - 5], points[n - 4], points[n - 3], points[n - 2], points[n - 1], points[n],
                points[n - 1], points[n], tension, continuity, bias))
        return curves
...
function math.angle_to_horizontal(r)
    ...
    if r > math.pi / 2 and r < 3 * math.pi / 2 then
        return -1
    elseif r >= 3 * math.pi / 2 or r <= math.pi / 2 then
        return 1
    end
end
...
function math.angle_to_vertical(r)
    ...
    if r > 0 and r < math.pi then
        return -1
    elseif r >= math.pi and r <= 2 * math.pi then
        return 1
    end
end
...
-- 使模的值在[1, module]之间，而不是[0, module]之间
function math.zmod(value, module)
...
function math.lerp(src, dst, amount)
    return src * (1 - amount) + dst * amount
end
...
-- px,py 是否在矩形内
function math.in_rectangle(px,py,x, y, w, h)
	return px - x > 0  and py - y > 0 and
		x + w - px > 0  and y + h - py > 0
end
