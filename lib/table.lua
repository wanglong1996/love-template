...
-- 将t2的元素插入t1，如果new为true则不影响t1
function table.append(t1, t2, new)
	local m = new and table.shallow_copy(t1) or t1
...
-- 使用t2的数据覆盖t1，t1，t2都有的会覆盖，t1没有，t2有的，t1会新增
function table.deepmerge(t1, t2, new)
	local m = new and table.copy(t1) or t1

	for k, v in pairs(t2) do
		v = new and table.copy(v) or v
...
-- 判断table是否为数组
function table.isarray(x)
    return type(x) == "table" and x[1] ~= nil
end

-- 删除表中指定对象
function table.remove_object(t, c)
    -- safe remove
