...
function Trigger:after(delay, action, tag)
    ...
    if type(delay) == 'number' or type(delay) == 'table' then
        self.triggers[tag] = { type = 'after', timer = 0, unresolved_delay = delay, delay = self:resolve_delay(delay), action =
        action }
    else
...
function Trigger:cooldown(delay, condition, action, times, after, tag)
    ...
    local after = after or function() end
    local tag = tag or random:uid()
    self.triggers[tag] = { type = 'cooldown', timer = 0, unresolved_delay = delay, delay = self:resolve_delay(delay), condition =
    condition, action = action, times = times, max_times = times, after = after, multiplier = 1 }
end
...
-- delay 可以是数字，table（取随机值）,条件语句
function Trigger:every(delay, action, times, after, tag)
    local times = times or 0
    local after = after or function() end
    local tag = tag or random:uid()
    if type(delay) == 'number' or type(delay) == 'table' then
        self.triggers[tag] = { type = 'every', timer = 0, unresolved_delay = delay, delay = self:resolve_delay(delay), action =
        action, times = times, max_times = times, after = after, multiplier = 1 }
    else
        self.triggers[tag] = { type = 'conditional_every', condition = delay, last_condition = false, action = action, times =
        times, max_times = times, after = after }
    end
end
...
function Trigger:during(delay, action, after, tag)
    ...
    local tag = tag or random:uid()
    if type(delay) == 'number' or type(delay) == 'table' then
        self.triggers[tag] = { type = "during", timer = 0, unresolved_delay = delay, delay = self:resolve_delay(delay), action =
        action, after }
    elseif type(delay) == 'function' then
        self.triggers[tag] = { type = 'conditional_during', condition = delay, last_condition = false, action = action, after =
        after }
    end
end
...
function Trigger:tween(delay, target, source, method, after, tag)
    ...
    self.triggers[tag] = { type = 'tween', timer = 0, unresolved_delay = delay, delay = self:resolve_delay(delay), target=target, initial_values =
    initial_values, source = source, method = method, after = after }
end
...
function Trigger:set_every_mutipler(tag, multiplier)
